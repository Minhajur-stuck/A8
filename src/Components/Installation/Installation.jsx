...
console.log(installed)
  ...
  const handleSort=(type)=>{
    ...
    if(type === 'highToLow'){
      const bigToSmall = [...installed].sort((a,b)=> b.downloads - a.downloads)
    ...
    if(type === 'lowToHigh'){
       ...
       const smallToBig = [...installed].sort((a,b)=> a.downloads - b.downloads)
  ...
  return (
    <div className="max-w-[1300px] mx-auto">
      ...
      <div className="flex items-center justify-between my-2">
        <div className="font-semibold text-2xl">{installed.length} apps Found</div>
        <details className="dropdown">
          <summary className="btn m-1">Sort By Download <img src={sortImg} alt="" /></summary>
          <ul className="menu dropdown-content bg-base-100 rounded-box z-1 w-[135px] p-2 shadow-sm">
            <li>
              <a onClick={()=> {handleSort('highToLow')}}>High-Low</a>
            </li>
            <li>
              <a  onClick={()=> {handleSort('lowToHigh')}}>Low-High</a>
            </li>
          </ul>
        </details>
       ...
       <div>
           {
            installed.map(app => <InstalledApp handleUninstall={handleUninstall} key={app.id} app={app}></InstalledApp>)
           }
        </div>
